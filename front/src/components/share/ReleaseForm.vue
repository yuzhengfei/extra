...
<template>
  <div class="release-form" v-loading="loading">
    ...
    <div>
      <span>奖励积分<i>*</i></span>
      <div :class="{error:$v.budget.$error }">
        <input type="text" placeholder="请输入奖励积分" v-model="integral" @input="$v.budget.$touch()">
      </div>
      <span v-if="$v.budget.$error && !$v.budget.required" class="validator-error">积分必须的</span>
      <span v-if="!$v.budget.numeric" class="validator-error">积分必须为数字</span>
    </div>
    <div>
      <span>项目周期<i>*</i></span>
